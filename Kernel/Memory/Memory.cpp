...
void init_kmalloc_eternal()
{
    ...
    pEternalMallocPointer  = reinterpret_cast<uint8_t *> (ALIGN_UP(reinterpret_cast<Address_t>(&__kernel_virtual_end), PAGE_SIZE));

    uint8_t *pBootInfoStruct = static_cast<uint8_t *>(VirtAddr(((Address_t) g_pBootInfo) + g_pBootInfo->total_size));

    if (pBootInfoStruct > pEternalMallocPointer)
        pEternalMallocPointer = reinterpret_cast<uint8_t *>((Address_t) pBootInfoStruct);

    pEternalMallocPointer = reinterpret_cast<uint8_t *>(ALIGN_UP((Address_t) pEternalMallocPointer, PAGE_SIZE));
...
void *kmalloc_eternal(const size_t size)
{
    ...
    pEternalMallocPointer = reinterpret_cast<uint8_t *>(ALIGN_UP((Address_t)pEternalMallocPointer, ALIGNMENT_BYTES));
    ...
    for (uint8_t *pAllocation = pAlloc; pAllocation < (pAlloc + size); pAllocation += PAGE_2M)
    {
        ...
        MM::Vmm::Get().KernelInitMap(PhysAddr::FromVirt(VirtAddr(pMappingMemory)).PageAddr(PAGE_2M),
                                    VirtAddr(pMappingMemory).PageAddr(PAGE_2M));
    }
...
void *kmalloc_eternal_aligned(const size_t size, size_t alignment)
{
    ...
    pEternalMallocPointer = reinterpret_cast<uint8_t *>(ALIGN_UP((Address_t) pEternalMallocPointer, alignment));
    MM::Vmm::Get().KernelInitMap(PhysAddr::FromVirt(VirtAddr(pEternalMallocPointer)).PageAddr(PAGE_2M),
                                    VirtAddr(pEternalMallocPointer).PageAddr(PAGE_2M));
...
void set_kmalloc_eternal_ptr(void *pKmallocEternal)
{
    ...
    pEternalMallocPointer = reinterpret_cast<uint8_t *>(ALIGN_UP((Address_t) pKmallocEternal, ALIGNMENT_BYTES));
}
...
{
    return BartOS::MM::Vmm::Get().GetHeapManager().Allocate(size);
}
...
{
    return BartOS::MM::Vmm::Get().GetHeapManager().Free(ptr);
}
...
{
    return BartOS::MM::Vmm::Get().GetHeapManager().Allocate(size, alignment);
}
