...
{
    VirtualAddress          m_vstart { 0 };
    VirtualAddress          m_vend { 0 };
    ...
    m_stackVmArea()
{
    ...
    size_t nPages = (pMultibootVmArea->m_vend - pMultibootVmArea->m_vstart).Get() / PhysicalPage::m_pageSize;
    pMultibootVmArea->m_physicalRegionList.push_back(Pmm::Get().AllocateRegion(PhysicalAddress(pMultibootVmArea->m_vstart), nPages));
    ...
    nPages = (pTextVmArea->m_vend - pTextVmArea->m_vstart).Get() / PhysicalPage::m_pageSize;
    pTextVmArea->m_physicalRegionList.push_back(Pmm::Get().AllocateRegion(PhysicalAddress(pTextVmArea->m_vstart), nPages));
    ...
    nPages = (pRoDataVmArea->m_vend - pRoDataVmArea->m_vstart).Get() / PhysicalPage::m_pageSize;
    pRoDataVmArea->m_physicalRegionList.push_back(Pmm::Get().AllocateRegion(PhysicalAddress(pRoDataVmArea->m_vstart), nPages));
    ...
    nPages = (pDataVmArea->m_vend - pDataVmArea->m_vstart).Get() / PhysicalPage::m_pageSize;
    pDataVmArea->m_physicalRegionList.push_back(Pmm::Get().AllocateRegion(PhysicalAddress(pDataVmArea->m_vstart), nPages));
    ...
    nPages = (pBssVmArea->m_vend - pBssVmArea->m_vstart).Get() / PhysicalPage::m_pageSize;
    pBssVmArea->m_physicalRegionList.push_back(Pmm::Get().AllocateRegion(PhysicalAddress(pBssVmArea->m_vstart), nPages));
    ...
    nPages = (pEternalHeapVmArea->m_vend - pEternalHeapVmArea->m_vstart).Get() / PhysicalPage::m_pageSize;
    pEternalHeapVmArea->m_physicalRegionList.push_back(Pmm::Get().AllocateRegion(PhysicalAddress(pEternalHeapVmArea->m_vstart), nPages));
    ...
    const VirtualAddress heapStart((Address_t) get_kmalloc_eternal_ptr());
    ...
    nPages = (pHeapVmArea->m_vend - pHeapVmArea->m_vstart).Get() / PhysicalPage::m_pageSize;
    pHeapVmArea->m_physicalRegionList.push_back(Pmm::Get().AllocateRegion(nPages));
    pKernelAddressSpace->AddVmArea(pHeapVmArea);

    //! Stack area.
    VmArea *pStackVmArea = new VmArea(VmAreaType::STACK, VirtualAddress(Vmm::TEMP_MAP_ADDR_BASE - INITIAL_STACK_SIZE), VirtualAddress(Vmm::TEMP_MAP_ADDR_BASE), *pKernelAddressSpace);
    pStackVmArea->m_pageFlags = (WRITABLE);
    pStackVmArea->m_vmAreaFlags = (VmAreaFlags::CAN_GROW | VmAreaFlags::GROWS_DOWN);
    nPages = (pStackVmArea->m_vend - pStackVmArea->m_vstart).Get() / PhysicalPage::m_pageSize;
    ...
    VmArea *pTempMapVmArea = new VmArea(VmAreaType::TEMP_MAPPING, VirtualAddress(Vmm::TEMP_MAP_ADDR_BASE), VirtualAddress(ADDRESS_SPACE_TOP), *pKernelAddressSpace);
