...
x86_64::PageTable *PhysicalAddressToPageTable(PhysicalAddress physicalAddress)
{
    return static_cast<x86_64::PageTable *>(VirtualAddress::Create(physicalAddress));
}
...
PhysicalAddress PageTableEntry::GetPhysicalAddress() const
{
    return m_pPte->GetPhysicalAddress();
}
...
Pml<ENTRY_TYPE, PML>::Pml(x86_64::PageTableEntry &pte) :
    Base(&pte),
    m_pPageTable(MM::Vmm::Get().GetKernelHeap().AllocatePml()),
...
Pml<ENTRY_TYPE, PML>::Pml(x86_64::PageTableEntry &pte, kernel_init_tag) :
    Base(&pte),
    m_pPageTable(PhysicalAddressToPageTable(pte.GetPhysicalAddress())),
    m_ownsPageTable(false),
    m_subDirectoryMap(m_hasher)
{
    //! Walk the page table and initialize the Pml.
...
{
    if (m_ownsPageTable)
        Vmm::Get().GetKernelHeap().FreePml(m_pPageTable);
}

// ---------------------------------------------------------------------------------------------------------

template <typename ENTRY_TYPE, PageMapLevel PML>
void Pml<ENTRY_TYPE, PML>::MapPage(const PhysicalAddress &physicalAddress, const VirtualAddress &virtualAddress,
        const PageFlags pageFlags, const PageSize pageSize)
{
    PageTableIndex index = virtualAddress.GetLevel<AddressBitField>();
    ...
    if (!isHugePage)
        pSubDirectory->MapPage(physicalAddress, virtualAddress, pageFlags, pageSize);
}
...
template <>
void Pml1::MapPage(const PhysicalAddress &physicalAddress, const VirtualAddress &virtualAddress,
        const PageFlags pageFlags, const PageSize pageSize)
{
